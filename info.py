...
USE_CAPTION_FILTER = _coerce_bool(environ.get('USE_CAPTION_FILTER', True), True)  # Use caption filter for search results (default: True)
INDEX_CAPTION = _coerce_bool(environ.get('SAVE_CAPTION', True), True) # Save caption db when idexing make it False if you dont use USE_CAPTION_FILTER for search results (default: True)
...
# ============================
MOVIE_UPDATE_NOTIFICATION = _coerce_bool(environ.get('MOVIE_UPDATE_NOTIFICATION', False), False)  # Notification On (True) / Off (False)
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-100'))  # Notification of sent to your channel
DREAMXBOTZ_IMAGE_FETCH = _coerce_bool(environ.get('DREAMXBOTZ_IMAGE_FETCH', True), True)  # On (True) / Off (False)
LINK_PREVIEW = _coerce_bool(environ.get('LINK_PREVIEW', False), False) # Shows link preview in notification msg instead of image
ABOVE_PREVIEW = _coerce_bool(environ.get('ABOVE_PREVIEW', True), True) # Shows link preview above the text in notification msg if True else below the msg
...
TMDB_POSTER = _coerce_bool(environ.get('TMDB_POSTER', False), False) # Shows TMDB poster in notification msg
LANDSCAPE_POSTER = _coerce_bool(environ.get('LANDSCAPE_POSTER', True), True) # Shows landscape poster in notification msg
...
NO_RESULTS_MSG = _coerce_bool(environ.get("NO_RESULTS_MSG", True), True)  # True if you want no results messages in Log Channel
...
PM_SEARCH = _coerce_bool(environ.get('PM_SEARCH', True), True)  # PM Search On (True) / Off (False)
EMOJI_MODE = _coerce_bool(environ.get('EMOJI_MODE', False), False)  # Emoji status On (True) / Off (False)
BUTTON_MODE = is_enabled((environ.get('BUTTON_MODE', "False")), False) # pm & Group button or link mode (True) / Off (False)
STREAM_MODE = _coerce_bool(environ.get('STREAM_MODE', True), True) # Set Stream mode True or False
PREMIUM_STREAM_MODE = _coerce_bool(environ.get('PREMIUM_STREAM_MODE', True), True) # Set Stream mode True or False only for premium users
...
NO_PORT = _coerce_bool(environ.get('NO_PORT', False), False)
APP_NAME = None
if 'DYNO' in environ:
    ON_HEROKU = True
...
if 'DYNO' in environ:
    ...
    ON_HEROKU = False
HAS_SSL = _coerce_bool(getenv('HAS_SSL', True), True)
if HAS_SSL:
